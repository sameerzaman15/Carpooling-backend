  ...
  async leaveGroup(groupId: number, userId: number): Promise<void> {
    return this.groupRepo.manager.transaction(async transactionalEntityManager => {
      ...
      if (group.owner.id === userId) {
        throw new ForbiddenException('The owner cannot leave the group. Transfer ownership or delete the group instead.');
      }
      ...
      await transactionalEntityManager
        ...
        .delete()
        .from('users_groups_group')
        .where("usersId = :userId AND groupId = :groupId", { userId, groupId })
        .execute();

      // Save the updated group
      await transactionalEntityManager.save(group);

      console.log(`User ${userId} has left group ${groupId} successfully`);
    });
  }
